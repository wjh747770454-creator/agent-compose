...
func ScanProjectRun(scan func(dest ...any) error) (domain.ProjectRunRecord, error) {
	...
	var updatedAtRaw any
	if err := scan(
		&item.RunID, &item.ParentRunID, &item.RootRunID, &item.DelegationID, &item.DelegationAttempt, &item.DelegationReason,
		&item.ProjectID, &item.ProjectName, &item.ProjectRevision, &item.AgentName, &item.ManagedAgentID, &item.Source, &item.SchedulerID, &item.TriggerID, &item.Status,
		&item.SandboxID, &item.ExitCode, &item.Error, &item.Prompt, &item.Output, &item.ResultJSON, &item.StructuredResultJSON, &item.LogsPath, &item.ArtifactsDir, &item.CleanupError, &item.Driver, &item.ImageRef,
...
func SelectProjectRunSQL() string {
	return `SELECT run_id, parent_run_id, root_run_id, delegation_id, delegation_attempt, delegation_reason,
