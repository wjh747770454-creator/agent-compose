...
当 `system_prompt` 为空时，host **删除** `system-prompt.txt`，避免同 sandbox 后续 run 读到过期的 identity 文本。

## Host（Go）实现

主要文件：`pkg/agentcompose/adapters/agent_runner.go`、`pkg/agentcompose/adapters/agent_executor.go`、
`pkg/agentcompose/adapters/loader_host.go`、`pkg/runs/controller.go`。

### 解析 agent system prompt

**函数：** `AgentRunner.resolveAgentSystemPrompt(ctx, sandbox, agentDefinitionID string) (string, error)`

解析顺序：

1. 若 `agentDefinitionID`（即 `ExecuteAgentRequest.AgentDefinitionID`）非空，直接加载该 agent definition。
2. 否则，若 sandbox 带有 `source=agent` 和 `agent_id=<uuid>` 标签，按标签中的 agent id 加载。
3. 否则返回 `""`（非错误）。
...
**函数：** `execution.WriteAgentSystemPromptFile(sandbox, systemPrompt string) error`

| 属性 | 值 |
| --- | --- |
| Host 路径 | `{hostSandboxDir}/state/agents/system-prompts/system-prompt.txt` |
...
- **v1 session chat 兼容运行**：未显式传入 id 时，依赖 sandbox 标签。

`buildAgentExecSpec` 仅传入 `--state-root`；guest 从该根路径下的约定路径发现 agent identity。

### 并发假设

固定文件名假设同 sandbox 内 agent run 不会并发写入不同的 agent identity。当前 UI/API 路径按 cell 串行执行 chat。若未来需要并发不同 identity，再评估 per-run 文件名或 sandbox 级锁。
...
| --- | --- |
| v1 agent session chat 兼容路径 | Sandbox 标签 `source=agent` + `agent_id` |
...
- 空 `system_prompt` 解析为 `""`
- 带 sandbox 标签的 agent 解析出 trim 后的 prompt 文本
...
- System prompt 文件与 per-turn prompt 文件同处 sandbox state 树，受相同 sandbox 生命周期与清理策略约束。
...
| 文件 | 变更 |
| --- | --- |
| `pkg/agentcompose/adapters/agent_runner.go` | `resolveAgentSystemPrompt`、`BuildAgentExecSpec`、agent prompt 文件处理 |
| `pkg/execution/model.go` | `ExecuteAgentRequest` 新增 `AgentDefinitionID` |
| `pkg/agentcompose/adapters/loader_host.go` | 向 agent 执行传递 agent definition id |
| `pkg/runs/controller.go` | 向 agent 执行传递 `ManagedAgentID` |
| `pkg/agentcompose/adapters/agent_runner_test.go` | host 解析、固定路径写入/删除测试 |
...
3. Codex provider thread 在 prompt 编辑后 resume 时使用新指令。
